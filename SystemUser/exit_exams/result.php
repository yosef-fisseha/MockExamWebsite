...
<body>
  ...
  <aside id="sidebar" class="sidebar">
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <li class="nav-item">
        <a class="nav-link " href="http://<?php echo $server_ip ?>/MockExamWebsit/SystemUser/home.php">
    ...
    <div class="maincontent">
        ...
        if(isset($_SESSION["answer"]))
        {
            for($i=0;$i<$que_num_size;$i++){
                $answer="";
                $query=mysqli_query($connect,"SELECT * FROM `$que_tbl_name` where model_or_exam = 'exam' && `exam_year` = $_SESSION[exam_year] && questionID = $que_num[$i]");
        ...
        <?php
        $query1=mysqli_query($connect,"SELECT * FROM `$que_tbl_name` where model_or_exam = 'exam' && `exam_year` = $_SESSION[exam_year]");
     ...
     {
        #$date = date("Y-m-d");
        mysqli_query($connect,"INSERT INTO `result`(`ID`, `user_name`, `dep_ID`, `category`, `exam_level`, `total_questions`, `correct_answer`, `wrong_answer`, `exam_time`) VALUES ('','$username','$dep_ID','Exit Exam','-','$count','$correct','$wrong','$date')");
     }
...
<h1 style="text-align: center;" >ANSWERS</h1>
        ...
        $query = mysqli_query($connect,"SELECT * FROM `$que_tbl_name` where model_or_exam = 'exam' && `exam_year` = $_SESSION[exam_year]");
        $q_num = 1;
